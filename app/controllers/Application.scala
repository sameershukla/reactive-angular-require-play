...
object Application extends Controller with Security {
  ...
  implicit val namesResponse = Json.writes[Names]
  
  implicit val nameReads: Reads[Names] = (
      (
        (JsPath \ "first").read(minLength[String](5)) and
        (JsPath \ "last").read(minLength[String](5)) 
   ))(Names.apply _)
  ...
  implicit val LoginCredentialsFromJson = (
    ...
    (__ \ "password").read[String](minLength[String](2)))((email, password) => LoginCredentials(email, password))
  ...
  def form = Action(parse.json) { request =>
    val json = request.body
    println(json)
    nameReads.reads(json).fold(
      invalid => BadRequest(JsError.toFlatJson(invalid)),
      valid => {
         val name = json.as[Names]
